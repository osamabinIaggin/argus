...
function BracketLines({
  width, height, bracketSize, visible, opacityRef,
}: {
  width: number; height: number; bracketSize: number; visible: boolean
  opacityRef: React.MutableRefObject<number>
}) {
  const z = 0.02
  const geometry = useMemo(() => {
    const hw = width / 2; const hh = height / 2; const s = bracketSize
    const positions = new Float32Array([
      -hw, hh, z, -hw, hh - s, z, -hw, hh, z, -hw + s, hh, z,
      hw - s, hh, z, hw, hh, z, hw, hh, z, hw, hh - s, z,
      hw, -hh + s, z, hw, -hh, z, hw, -hh, z, hw - s, -hh, z,
      -hw + s, -hh, z, -hw, -hh, z, -hw, -hh, z, -hw, -hh + s, z,
    ])
  ...
  return (
    <lineSegments geometry={geometry}>
      <lineBasicMaterial ref={matRef} color="white" transparent opacity={0} depthTest={false} />
    </lineSegments>
  )
}
...
export function VideoPlane({ videoSrc, index, isHovered, deformStrength, onHover, onLeave }: VideoPlaneProps) {
  ...
  useEffect(() => {
    ...
    setUniforms({ uMap: { value: texture }, uDeform: { value: 0 }, uTime: { value: 0 } })
  ...
  return (
    <group position={[offsetX, offsetY, 0]}>
      <mesh ref={meshRef} onPointerEnter={handlePointerEnter} onPointerLeave={handlePointerLeave}>
      ...
      <BracketLines
        width={planeWidth} height={planeHeight} bracketSize={bracketSize}
        visible={isHovered} opacityRef={bracketOpacityRef}
      />
      {isHovered && (
        <Html
          position={[planeWidth / 2 - 0.15, -planeHeight / 2 + 0.15, 0.03]}
          transform
          style={{ pointerEvents: 'none', fontSize: '10px', fontFamily: 'system-ui, sans-serif', color: 'rgba(255,255,255,0.7)', whiteSpace: 'nowrap' }}
        >
